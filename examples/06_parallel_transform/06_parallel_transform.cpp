...
TEST_CASE("std::transform", "parallel_transform") {
...
TEST_CASE("cppcon::sequential::transform", "parallel_transform") {
  auto input = std::vector<int>(size);
  auto output = std::vector<int>(size);
...
TEST_CASE("cppcon::parallel::transform", "parallel_transform") {
  auto input = std::vector<int>(size);
  auto output = std::vector<int>(size);
  ...
  auto time = eval_performance(
      [&]() {
        cppcon::parallel::transform(input.begin(), input.end(),
                                      output.begin(),
                                      [](int &in) { return in * 2; });
      },
      iterations);
  ...
  print_time<std::milli>(
      "cppcon::parallel::transform (" + std::to_string(iterations) + " iterations)", time);
