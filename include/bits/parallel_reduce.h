...
T reduce(par_execution_policy_t policy, ForwardIt first, ForwardIt last, T init,
  ...
  for (unsigned t = 0; t < (concurrency - 1); t++) {
    diff_t currentChunkSize =
        elementsPerThread + static_cast<diff_t>((t + 1) < remainder);
    diff_t currentOffset = ((t + 1) * elementsPerThread) +
                           std::min(static_cast<diff_t>(t + 2), remainder);
    threads.emplace_back(
        [t, processChunk = std::move(processChunk),
         ...
         &partialReductions]() mutable {
          partialReductions[t + 1] = processChunk(c_first, c_last);
        });
  }
  diff_t currentChunkSize =
  ...
  return processChunk(partialReductions.begin(), partialReductions.end());
}
